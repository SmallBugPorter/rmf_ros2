...
namespace rmf_fleet_adapter
{
    namespace events
    {
        ...
        void GoToPlace::Active::_find_plan()
        {
            ...
            if (!_chosen_goal.has_value())
            {
                std::string error_msg = "Unable to find a path to any of the goal options [" + _description.destination_name(*_context->task_parameters()) + "]";
                                             ...
                                             const services::FindPath::Result &result)
                                         {
                                             ...
                                             if (!result)
                                             {
                                                 ...
                                                 self->_state->update_log().error(
                                                     "Failed to find a plan to move from [" + start_name + "] to [" + goal_name + "]. Will retry soon.");
                                                 ...
                                                 self->_context->worker()
                                                     .schedule([update = self->_update](const auto &)
                                                               { update(); });
                                             ...
                                             self->_state->update_log().info(
                                                 "Found a plan to move from [" + start_name + "] to [" + goal_name + "]");
                ...
                for (std::size_t i = 0; i < waypoints.size(); ++i)
                {
                    ...
                    for (std::size_t j = 0; j < wp.progress_checkpoints().size(); ++j)
                    {
                        ...
                        plan_dump
                            ...
                            << ", planned_arrival_time_s="
                            << rmf_traffic::time::to_seconds(
                                   progress.time.time_since_epoch())
                            << ", checkpoints=[";
                     ...
                     ++route_index)
                {
                    ...
                    for (const auto &trajectory_wp : trajectory)
                    {
                        ...
                        plan_dump
                            ...
                            << " planned_arrival_time_s="
                            << rmf_traffic::time::to_seconds(
                                   trajectory_wp.time().time_since_epoch())
                            << ", position=(x=" << p.x()
                            << ", y=" << p.y()
                            << ", yaw=" << p.z() << ")"
                            << ", velocity=(vx=" << v.x()

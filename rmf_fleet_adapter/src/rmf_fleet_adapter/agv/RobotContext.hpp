...
namespace rmf_fleet_adapter
{
    ...
    namespace agv
    {
        ...
        {
            std::stringstream ss;
            ss << std::fixed << std::setprecision(4) << std::left;

            // 时间列, 固定宽度 12 (t=XX.XXXX)
            ss << "t=" << std::setw(10)
               << rmf_traffic::time::to_seconds(wp.time() - t0);

            // 图索引列, 固定宽度 7 (#XXX)
            if (wp.graph_index().has_value())
                ss << "  #" << std::setw(4) << *wp.graph_index();
            else
                ss << "  " << std::setw(5) << "-";
            ...
            {
                const auto &loc =
                    graph.get_waypoint(*wp.graph_index()).get_location();
                ss << "  (" << std::right << std::setw(8) << loc.x()
                   << "," << std::right << std::setw(8) << loc.y() << ")";
            }
            else
            {
            ...
            // 航向列, 从第一个 approach lane 的方向推导
            if (wp.graph_index().has_value() && !wp.approach_lanes().empty())
            {
                const auto &lane = graph.get_lane(wp.approach_lanes().front());
                const auto &entry_loc =
                    graph.get_waypoint(lane.entry().waypoint_index())
                        .get_location();
                const auto &exit_loc =
                    graph.get_waypoint(lane.exit().waypoint_index())
                        .get_location();
                const double dx = exit_loc.x() - entry_loc.x();
                const double dy = exit_loc.y() - entry_loc.y();
                const double yaw = std::atan2(dy, dx);
                ss << "  yaw=" << std::left << std::setw(8) << yaw;
            }
            else
            {
                ss << "  yaw=" << std::left << std::setw(8) << "-";
            }

...
namespace rmf_fleet_adapter
{
        ...
        std::weak_ptr<agv::FleetUpdateHandle> fleet_handle)
    {
        ...
        mgr->_task_request_api_sub = mgr->_context->node()->task_api_request().observe_on(rxcpp::identity_same_worker(mgr->_context->worker())).subscribe([w = mgr->weak_from_this()](const rmf_task_msgs::msg::ApiRequest::SharedPtr &request)
                                                                                                                                                          {
      if (const auto self = w.lock())
        ...
        std::function<void(std::vector<std::string>)> on_failure)
    {
        ...
        fleet_impl.reassign_dispatched_tasks(
            ...
            [name = _context->requester_id(),
             ...
             fleet,
             on_failure,
             self = shared_from_this()](std::vector<std::string> errors)
            {
    ...
    TaskManager::RobotModeMsg TaskManager::robot_mode() const
    {
        const auto mode = rmf_fleet_msgs::build<RobotModeMsg>()
                              .mode(_active_task.is_finished() ? RobotModeMsg::MODE_IDLE : _context->current_mode())
                              .mode_request_id(0);
        ...
        const std::string &request_id)
    {
        ...
        {
            return _make_error_response(
                20, "Uncommissioned", "The robot [" + robot + "] in fleet [" + fleet + "] is not commissioned to perform direct tasks.");
        }
        ...
        if (!task_planner)
        {
            RCLCPP_ERROR(
                _context->node()->get_logger(),
                "车队 [%s] 未配置任务规划参数。"
                "请使用 FleetUpdateHandle::set_task_planner_params(~) 设置所需参数。",
                fleet.c_str());
    ...
    void TaskManager::_begin_next_task()
    {
        ...
        const auto assignment = is_next_task_direct ? _direct_queue.begin()->assignment : _queue.front();
    ...
    {
        _finished_waiting = false;
        auto task_id = "emergency_pullover." + _context->name() + "." + _context->group() + "-" + std::to_string(_count_emergency_pullover++);
    ...
    void TaskManager::_begin_waiting()
    {
        ...
        const auto task_id = "wait." + _context->name() + "." + _context->group() + "-" + std::to_string(_count_waiting++);
        ...
        TaskManager::TaskSummaryMsg &msg)
    {
        if (task == nullptr) // ResponsiveWait
        {
            ...
            msg.end_time = _queue.empty() ? msg.start_time : rmf_traffic_ros2::convert(_queue.front().deployment_time());
        ...
        const nlohmann::json_uri &id, nlohmann::json &value) const
    {
        ...
        if (it == _schema_dictionary.end())
        {
            RCLCPP_ERROR(
                _context->node()->get_logger(),
                "[TaskManager] 在 schema 字典中未找到 URL: %s", id.url().c_str());
            return;
        }
        ...
        const nlohmann::json_schema::json_validator &validator) const
    {
        ...
        if (!client)
        {
            RCLCPP_ERROR(
                _context->node()->get_logger(),
                "无法锁定机器人 [%s] 的 TaskManager 中的 BroadcastClient",
                _context->name().c_str());
            return;
        }
        client->publish(msg);
        ...
        const std::string &request_id)
    {
        ...
        if (direct_policy_it != request_json.end() && direct_policy_it->get<std::string>() == "cancel")
        {
        ...
        if (dispatch_policy_it == request_json.end() || dispatch_policy_it->get<std::string>() == "reassign")
        {

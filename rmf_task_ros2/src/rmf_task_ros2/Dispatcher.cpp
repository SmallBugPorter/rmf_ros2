...
namespace rmf_task_ros2
{
    ...
    namespace
    {
        ...
        //==============================================================================
        static const std::function<void(const nlohmann::json_uri &id, nlohmann::json &value)>
    ...
    public:
            ...
            : node{std::move(node_)}
        {
            ...
            dispatch_command_timer = node->create_wall_timer(
                ...
                [this]()
                { this->publish_lingering_commands(); });
        ...
        void handle_api_request(const ApiRequestMsg &msg)
        {
            ...
            try
            {
                ...
                std::string task_id =
                    task_request_json["category"].get<std::string>() + ".dispatch-";
        ...
        bool cancel_task(const TaskID &task_id)
        {
            ...
            {
                if (finished_it->second->status == Status::FailedToAssign || finished_it->second->status == Status::CanceledInFlight)
                {
